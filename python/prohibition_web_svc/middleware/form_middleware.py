...
def lease_a_form_id(**kwargs) -> tuple:
    logger.verbose('inside lease_a_form_id()')
...
def renew_form_id_lease(**kwargs) -> tuple:
    logger.verbose('inside renew_form_id_lease()')
    ...
    try:
        ...
        form = db.session.query(Form) \
            ...
            .first()
        if form is None:
            logger.warning('User, {}, cannot renew the lease on {} form'.format(user_guid, form_id))
            record_error(
                        **{
...
def list_all_users_forms(**kwargs) -> tuple:
    user_guid = kwargs.get('user_guid')
    all_ids = []
    logger.verbose("inside list_all_forms() {}".format(user_guid))
    try:
...
def admin_list_all_forms_by_type(**kwargs) -> tuple:
    logger.verbose("inside admin_list_all_forms()")
    form_type = kwargs.get('form_type')
    try:
        all_forms = db.session.query(Form) \
            .filter(Form.form_type == form_type) \
...
def validate_form_payload(**kwargs) -> tuple:
    ...
    if v.validate(kwargs.get('payload')):
        return True, kwargs
    logger.warning("validation error: " + json.dumps(v.errors))
    kwargs['validation_errors'] = v.errors
    return False, kwargs


def admin_create_form(**kwargs) -> tuple:
    logger.verbose("inside admin_create_form()")
    payload = kwargs.get('payload')
    logger.warning(str(payload))
    try:

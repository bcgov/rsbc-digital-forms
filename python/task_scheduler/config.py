...
class Config():
    ...
    LOG_FORMAT = "[DF_TASK_SCHED] %(asctime)s::%(levelname)s::%(name)s::%(message)s"
    ...
    LOGGING = {
        ...
        'formatters': {
            'json': {
                ...
                'format': '[DF_TASK_SCHED] %(asctime)s %(filename)s %(funcName)s %(levelname)s %(lineno)d %(module)s %(message)s %(pathname)s'
            },
            'brief': {
                'format': LOG_FORMAT
            }

...
export const validationSchema = Yup.object().shape({
  ...
  "driver_phone": Yup.string().matches(/^\d{3}-\d{3}-\d{4}$/, 'Phone number format ###-###-####'),
  ...
  "reason_for_not_impounding": Yup.string()
  .test('not_impounded', 'Reason for not Impounding is required', function(value) {
  ...
  "vehicle_released_to": releasedToDriverValidation(Yup.ref('reason-for-not-impounding')),
  ...
  "time_released": releasedToDriverValidation(Yup.ref('reason_for_not_impounding'))
  .matches(/^([01]\d|2[0-3])[0-5]\d$/, 'Invalid time format'),
  "test_used_alcohol": prescribedDeviceValidation(Yup.ref('prescribed_test_used')),
  ...
  .test('bac_result_mg', 'bac_result_mg is required when instrument is selected', function (value) {
  ...
  .test('ASD', 'ASD expiry date is required when ASD is selected', function (value) {
    const selectedValue = this.parent['test_used_alcohol'];
  ...
  .nullable()
  .test('result_alcohol', 'Test result is required when alco-sensor is selected', function(value) {
    const selectedValue = this.parent['test_used_alcohol'];
  ...
  "test_used_drugs": prescribedDeviceValidation(Yup.ref('prescribed_test_used')),
});
...
export const TestAdministered = (props) => {
    ...
    return(
        <>{values["prescribed_test_used"] === 'YES' && (<div className='border-design-form left text-font'>
            <h3 >Test Administered - {values["type_of_prohibition"] === "alcohol" ? 'Alcohol 215(2)' : 'Drugs 215(3)'}</h3>
        {values["type_of_prohibition"] === "alcohol" && (
            <div className="test-admin-alcohol">
                <div className="row">
                    <div className="col">
                        <Radio label= "Which test was used?" name="test_used_alcohol" options={[
                ...
                {(values["test_used_alcohol"] === "alco-sensor") &&<div className="row">
                     <div className="col-sm-12">
                        <DatePickerField name="asd_expiry_date" label="ASD Expiry Date" className="field-height field-width" required/>
                        <Radio label= "Result" name="result_alcohol" options={[
                                { label: '51-99 mg%', value: '51-99 mg%' },
                                { label: 'Over 99 mg%', value: 'Over 99 mg%' },
                                ]} required />
                    </div>
                </div>}
                {values["test_used_alcohol"] ==="instrument" && <div className="row">
                    <div className="col-sm-12 mt-2">
                        <NumericInput label="BAC Result(mg%)" name="bac_result_mg" required/>
                    </div>
                </div>}
            </div>)}
            ...
            <div className="test-admin-drug">
                <div className="row">
                    <div className="col-sm-12">
                        <Radio label= "Which test was used?" name="test_used_drugs" options={[

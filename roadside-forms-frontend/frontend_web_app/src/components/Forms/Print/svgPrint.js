            ...
            <svg viewBox={viewBox} xmlns='http://www.w3.org/2000/svg' className={'svg-wrapper' + formAspect}>
                ...
                {formFields?.map( (item) => {
                    if(allFormFields[item]['field_type'] === "text"){
                        ...
                        return (<foreignObject id={item} x={allFormFields[item]["start"]["x"]+"px"} y={allFormFields[item]["start"]["y"]+"px"} className={allFormFields[item]["classNames"]} fill="black">{printFormatHelper(values,allFormFields[item], item)}</foreignObject>)
                    }
                    return null
                })}
                )
